...
<!--      <xsl:variable name="id" select="tokenize(id,':')[last()]"/>-->
      ...
      <xsl:choose>
         ...
         <xsl:otherwise><xsl:value-of select="*:distribution/@ProviderName"/></xsl:otherwise>
      </xsl:choose>
      <xsl:value-of select="$delim1"/>
      <xsl:value-of select="normalize-space(tokenize(../title, '\|')[1])"/>
      <xsl:value-of select="$delim1"/>
      <xsl:text>"</xsl:text><xsl:value-of select="normalize-space(translate(title, '\&quot;', ''))"/><xsl:text>"</xsl:text>
      <xsl:value-of select="$delim1"/>
      <xsl:value-of select="normalize-space(id)"/>
      <xsl:value-of select="$delim1"/>
      <xsl:value-of select="$base_url"/><xsl:value-of select="encode-for-uri(id)"/>
      <xsl:value-of select="$delim1"/>
